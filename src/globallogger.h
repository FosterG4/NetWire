...
enum class LogLevel {
    ...
    FATAL = 6       // Fatal errors (application will exit)
};
...
#define LOG_FUNCTION_ENTRY() \
    LOG_DEBUG(QString("ENTER: %1").arg(Q_FUNC_INFO)); \
    struct FunctionExitLogger { \
        QString funcInfo; \
        FunctionExitLogger(const QString& f) : funcInfo(f) {} \
        ~FunctionExitLogger() { GlobalLogger::instance()->debug(QString("EXIT: %1").arg(funcInfo), Q_FUNC_INFO, __FILE__, __LINE__); } \
...
#define LOG_PERFORMANCE_START(operation) \
    QElapsedTimer* timer_##operation = new QElapsedTimer(); \
    timer_##operation->start(); \
    LOG_DEBUG(QString("PERFORMANCE START: %1").arg(#operation))

#define LOG_PERFORMANCE_END(operation) \
    if (timer_##operation) { \
        qint64 elapsed = timer_##operation->elapsed(); \
        LOG_DEBUG(QString("PERFORMANCE END: %1 - %2ms").arg(#operation).arg(elapsed)); \
        delete timer_##operation; \
        timer_##operation = nullptr; \
    }

// Memory logging
#define LOG_MEMORY_ALLOC(size) LOG_DEBUG(QString("MEMORY ALLOC: %1 bytes").arg(size))
#define LOG_MEMORY_FREE(size) LOG_DEBUG(QString("MEMORY FREE: %1 bytes").arg(size))

// Thread logging
#define LOG_THREAD_START() \
    GlobalLogger::instance()->setThreadName(QString("Thread-%1").arg(QThread::currentThreadId())); \
    LOG_DEBUG("Thread started")

#define LOG_THREAD_END() LOG_DEBUG("Thread ended")

// ========================================
// DEBUG INTEGRATION
// ========================================
// Replace old debug macros with new logging system
